...
4. Keep Framework Preset as Next.js. `frontend/vercel.json` performs locked installs for `services` and `frontend` independently. The Next.js trace root stays anchored to `frontend`, matching Vercel's configured application root.
5. Use Node.js 22.x.
